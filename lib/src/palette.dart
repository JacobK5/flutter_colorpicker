  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
        ...
        child: ColorPickerSlider(
          ...
          fullThumbColor: widget.fullThumbColor,
          customColors: widget.customColors,
          onColorReleased: onColorReleased,
        ));
  }
}

class ColorPickerSlider extends StatefulWidget {
  const ColorPickerSlider(this.trackType, this.hsvColor, this.onColorChanged,
      {Key? key,
      this.displayThumbColor = false,
      this.fullThumbColor = false,
      this.customColors = const [],
  ...
  final bool fullThumbColor;
  final List<Color> customColors;
  final ValueChanged<Color>? onColorReleased;
...
class _ColorPickerSliderState extends State<ColorPickerSlider> {
  ...
  void internalOnColorChanged(HSVColor color, bool released) {
    widget.onColorChanged(color);
    if (released && widget.onColorReleased != null) {
      widget.onColorReleased!(color.toColor());
    }
  }

  void slideEvent(
      RenderBox getBox, BoxConstraints box, Offset? globalPosition) {
    bool released = false;
    ...
    // for now outside of the switch, will likely move it inside
    switch (widget.trackType) {
      case TrackType.hue:
        // 360 is the same as zero
        // if set to 360, sliding to end goes to zero
        internalOnColorChanged(
            widget.hsvColor.withHue(progress * 359), released);
        break;
      case TrackType.saturation:
        internalOnColorChanged(
            widget.hsvColor.withSaturation(progress), released);
        break;
      case TrackType.saturationForHSL:
        internalOnColorChanged(
            hslToHsv(hsvToHsl(widget.hsvColor).withSaturation(progress)),
            released);
        break;
      case TrackType.value:
        internalOnColorChanged(widget.hsvColor.withValue(progress), released);
        break;
      case TrackType.lightness:
        internalOnColorChanged(
            hslToHsv(hsvToHsl(widget.hsvColor).withLightness(progress)),
            released);
        break;
      case TrackType.red:
        internalOnColorChanged(
            HSVColor.fromColor(
                widget.hsvColor.toColor().withRed((progress * 0xff).round())),
            released);
        break;
      case TrackType.green:
        internalOnColorChanged(
            HSVColor.fromColor(
                widget.hsvColor.toColor().withGreen((progress * 0xff).round())),
            released);
        break;
      case TrackType.blue:
        internalOnColorChanged(
            HSVColor.fromColor(
                widget.hsvColor.toColor().withBlue((progress * 0xff).round())),
            released);
        break;
      case TrackType.alpha:
        internalOnColorChanged(
            widget.hsvColor.withAlpha(localDx.clamp(0.0, box.maxWidth - 30.0) /
                (box.maxWidth - 30.0)),
            released);
        break;
      case TrackType.white:
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
      ...
      switch (widget.trackType) {
        case TrackType.hue:
          thumbOffset += (box.maxWidth - 30.0) * widget.hsvColor.hue / 360;
          thumbColor =
              HSVColor.fromAHSV(1.0, widget.hsvColor.hue, 1.0, 1.0).toColor();
          break;
        case TrackType.saturation:
          thumbOffset += (box.maxWidth - 30.0) * widget.hsvColor.saturation;
          thumbColor = HSVColor.fromAHSV(
                  1.0, widget.hsvColor.hue, widget.hsvColor.saturation, 1.0)
              .toColor();
          break;
        case TrackType.saturationForHSL:
          thumbOffset +=
              (box.maxWidth - 30.0) * hsvToHsl(widget.hsvColor).saturation;
          thumbColor = HSLColor.fromAHSL(1.0, widget.hsvColor.hue,
                  hsvToHsl(widget.hsvColor).saturation, 0.5)
              .toColor();
          break;
        case TrackType.value:
          thumbOffset += (box.maxWidth - 30.0) * widget.hsvColor.value;
          thumbColor = HSVColor.fromAHSV(
                  1.0, widget.hsvColor.hue, 1.0, widget.hsvColor.value)
              .toColor();
          break;
        case TrackType.lightness:
          thumbOffset +=
              (box.maxWidth - 30.0) * hsvToHsl(widget.hsvColor).lightness;
          thumbColor = HSLColor.fromAHSL(1.0, widget.hsvColor.hue, 1.0,
                  hsvToHsl(widget.hsvColor).lightness)
              .toColor();
          break;
        case TrackType.red:
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().red / 0xff;
          thumbColor = widget.hsvColor.toColor().withOpacity(1.0);
          break;
        case TrackType.green:
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().green / 0xff;
          thumbColor = widget.hsvColor.toColor().withOpacity(1.0);
          break;
        case TrackType.blue:
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().blue / 0xff;
          thumbColor = widget.hsvColor.toColor().withOpacity(1.0);
          break;
        case TrackType.alpha:
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().opacity;
          thumbColor =
              widget.hsvColor.toColor().withOpacity(widget.hsvColor.alpha);
          break;
        case TrackType.white:
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().opacity;
          ...
          break;
        case TrackType.custom:
          // using opacity as the measure for now since it isn't really needed for lights
          thumbOffset +=
              (box.maxWidth - 30.0) * widget.hsvColor.toColor().opacity;
          thumbColor = widget.hsvColor
              .toColor(); //.withOpacity(widget.hsvColor.alpha); // maybe make this a passable variable too
          break;
      }
            ...
            child: Transform.translate(
              ...
              child: CustomPaint(
                painter: ThumbPainter(
                  thumbColor: widget.displayThumbColor ? thumbColor : null,
                  fullThumbColor: widget.fullThumbColor,
                ),
              ),
            ),
  ...
  void _handleGesture(
    ...
    if (widget.paletteType == PaletteType.hueWheel) {
      ...
      _handleColorWheelChange(
          ((rad + 90) % 360).clamp(0, 360), dist.clamp(0, 1), released);
    } else {
              ...
              (_AlwaysWinPanGestureRecognizer instance) {
                instance
                  ...
                  ..onUpdate = ((details) => _handleGesture(
                      details.globalPosition, context, height, width))
                  ..onEnd = ((details) {
                    _handleGesture(null, context, height, width);
                  });
              },
            ),
          },
